...
(function() {
  ...
  // ⏰ AI 试用到期弹窗
  const aiExpiry = '${user.ai_package_expires_at || ''}';
  if (aiExpiry) {
    const now = new Date();
    const expDate = new Date(aiExpiry);
    ...
    if (daysLeft <= 0) {
      setTimeout(() => {
        ...
        overlay.innerHTML = '<div class="modal" style="max-width:450px;"><h3>⏰ AI 试用已到期</h3><p style="color:#9ca3af;font-size:14px;margin:16px 0;">您3天的 AI 免费试用已结束。升级为付费套餐继续使用全部功能。</p><div style="margin:12px 0;"><a href="/company/packages" style="display:block;padding:12px;background:#2563eb;color:#fff;border-radius:8px;text-align:center;text-decoration:none;font-weight:600;">💎 查看套餐</a></div><button class="btn btn-outline" onclick="document.getElementById(\'trialExpiredModal\').parentElement.removeChild(document.getElementById(\'trialExpiredModal\'))" style="width:100%;justify-content:center;">稍后再说</button></div>';
      ...
      setTimeout(() => {
        const overlay = document.createElement('div');
        overlay.className = 'modal-overlay show';
        overlay.id = 'trialWarningModal';
        overlay.innerHTML = '<div class="modal" style="max-width:450px;"><h3>⏳ AI 试用即将到期</h3><p style="color:#9ca3af;font-size:14px;margin:16px 0;">您3天的 AI 免费试用还剩 <strong style="color:#f59e0b;">' + daysLeft + '天</strong>，到期后需购买套餐继续使用。</p><div style="margin:12px 0;"><a href="/company/packages" style="display:block;padding:12px;background:#f59e0b;color:#fff;border-radius:8px;text-align:center;text-decoration:none;font-weight:600;">💎 查看套餐</a></div><button class="btn btn-outline" onclick="document.getElementById(\'trialWarningModal\').parentElement.removeChild(document.getElementById(\'trialWarningModal\'))" style="width:100%;justify-content:center;">知道了</button></div>';
        document.body.appendChild(overlay);
      }, 1000);
    }
  }
